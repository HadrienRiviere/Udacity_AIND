...
#         raise NotImplementedError
        effectA = actionA.effects
        effectB = actionB.effects
        inconsistentA = [~effect in effectA for effect in effectB] 
        inconsistentB =  [~effect in effectB for effect in effectA]
        return any(inconsistentA + inconsistentB)
...
#         raise NotImplementedError
        ...
        testA = any([~effect in precondA for effect in effectB])
        testB = any([~effect in precondB for effect in effectA])
...
#         raise NotImplementedError
        precondA = actionA.preconditions
        precondB = actionB.preconditions
        testA = any(self.parent_layer.is_mutex(pA,pB) for pA in precondA for pB in precondB)
        testB = any(self.parent_layer.is_mutex(pB,pA) for pB in precondB for pA in precondA)
        return testA or testB
...
#         raise NotImplementedError
        ...
        testA = all(self.parent_layer.is_mutex(lA,lB) for lA in literalsA for lB in literalsB)
        testB = all(self.parent_layer.is_mutex(lB,lA) for lB in literalsB for lA in literalsA)
    ...
    def _negation(self, literalA, literalB):
        ...
        # TODO: implement this function
#         raise NotImplementedError
        testA = (literalA == ~literalB)
        testB = (literalB == ~literalA)
        return testA or testB
...
#         raise NotImplementedError
        ...
        while not self._is_leveled:
            ...
            if all_goal_met and goals_are_mutex : 
                return pg_level
            self._extend()
            pg_level+=1
        pg_level = -1 
